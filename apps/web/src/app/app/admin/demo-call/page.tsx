  ...
  async function fire(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/calls/demo", {
        ...
        body: JSON.stringify({ phone: phone.trim(), script }),
      });
      if (!res.ok) {
        const body = (await res.json().catch(() => ({}))) as { error?: string };
        throw new Error(body.error ?? `${res.status} ${res.statusText}`);
      }
      const body = (await res.json()) as { callSid: string; status: string };
      setState({ name: "ok", callSid: body.callSid, status: body.status });
    } catch (err) {
